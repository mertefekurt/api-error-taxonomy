...
from api_error_taxonomy import __version__
from api_error_taxonomy.core import (
    audit_records,
    read_records,
    render_json,
    render_markdown,
...
from api_error_taxonomy.rules import PROJECT_NAME


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog='api-error-taxonomy',
        description=(
    ...
    parser.add_argument("input", nargs="?", type=Path, help='API error taxonomy')
    parser.add_argument(
        "--format",
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"api-error-taxonomy: error: {exc}", file=sys.stderr)
        return 1
